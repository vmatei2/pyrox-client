...
`codewiki_docs/` is machine-generated by the `/codewiki` skill and is not part
of the wiki contract above — never hand-edit it, and never let it edit `wiki/`
or `docs/`. A SessionStart hook reports when it has drifted; when it fires,
tell the user the docs are stale (baseline, changed files, affected modules)
and ask whether to regenerate — don't run `/codewiki` unprompted, and don't
stay silent about it either. Precedence when sources disagree: code, then
`docs/`, then `wiki/`, then `codewiki_docs/`.
